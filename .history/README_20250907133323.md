...
NoobNinja is a viewer for neural network, deep learning and machine learning models (forked and enhanced from Netron).
...
**macOS**: Check the project's GitHub Releases for NoobNinja or build locally; or run `brew install --cask netron` for the upstream Netron package.
...
**Browser**: You can host the `source/` folder (e.g., with a static server) to run NoobNinja in the browser.
