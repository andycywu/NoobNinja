...
export class ModelSummarizer {
    ...
    summarize(model) {
        ...
        // 為不同 Graph 實作統一 inputs/outputs 屬性
        graph.inputs = graph.inputs || graph.input || graph._inputs || graph._input || [];
        graph.outputs = graph.outputs || graph.output || graph._outputs || graph._output || [];
