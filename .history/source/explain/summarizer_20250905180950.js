...
export class ModelSummarizer {
    ...
    summarize(model) {
        ...
        // 支援多種 Netron 模型結構 (一些 factory 使用 modules / graph.input / graph.output)
        let graphs = null;
        if (model && model._modules && model._modules.length > 0) {
            ...
            // 常見的 factory 使用 modules
            graphs = model.modules;
            console.log('✅ [ModelSummarizer] 使用 modules 結構，找到', graphs.length, '個模組');
        ...
        const graph = graphs[0]; // 使用第一個 graph / module
        ...
        // 暴露到全域變數，方便在瀏覽器 DevTools 或外部腳本快速存取完整 summary
        try {
    ...
    _extractInputs(graph) {
        ...
        // 支援直接傳入 inputs 陣列或 graph 物件
        const actualInputs = Array.isArray(graph) ? graph : (graph.inputs || graph.input || graph._inputs || graph._input || []);
        console.log('🔍 [ModelSummarizer] resolved inputs:', actualInputs);
        if (actualInputs) {
    ...
    _extractOutputs(graph) {
        ...
        const actualOutputs = Array.isArray(graph) ? graph : (graph.outputs || graph.output || graph._outputs || graph._output || []);
        console.log('🔍 [ModelSummarizer] resolved outputs:', actualOutputs);
        if (actualOutputs) {
    ...
    _buildNetronRaw(tensor, shape, dtype) {
        // 優先使用原始 toString()，但如果回傳 '[object Object]' 則改用 dtype[shape]
        try {
            const s = tensor && typeof tensor.toString === 'function' ? tensor.toString() : null;
        ...
        if (dtype && Array.isArray(shape) && shape.length > 0) {
            const shapeStr = shape.map((d) => (d === undefined || d === null) ? '?' : String(d)).join(',');
        ...
        try {
            return JSON.stringify(tensor, (k, v) => (typeof v === 'function' ? undefined : v));
        } catch (e) {
    ...
    _parseShape(tensor) {
        ...
        // 通用形狀解析策略 - 基於實際 Netron 資料結構
        const shapeExtractors = [
            // Netron Argument/Value 結構：argument.value -> [ Value ] 或 value 是單一 Value
            () => {
                if (Array.isArray(tensor.value) && tensor.value[0]?.type?.shape?.dimensions) {
                    return tensor.value[0].type.shape.dimensions.map(this._normalizeDimension.bind(this));
                }
                if (tensor.value && tensor.value?.type?.shape?.dimensions) {
                    return tensor.value.type.shape.dimensions.map(this._normalizeDimension.bind(this));
                }
                // 有時候 value 裡面包 initializer
                if (Array.isArray(tensor.value) && tensor.value[0]?.initializer?.type?.shape?.dimensions) {
                    return tensor.value[0].initializer.type.shape.dimensions.map(this._normalizeDimension.bind(this));
                }
                return null;
            },
            // Netron 標準結構：tensor.arguments[0].type.shape.dimensions
            () => {
                if (tensor.arguments && tensor.arguments[0]?.type?.shape?.dimensions) {
                    return tensor.arguments[0].type.shape.dimensions.map(this._normalizeDimension.bind(this));
                }
                return null;
            },

            // ONNX 格式
            () => tensor.type?.shape?.dimensions?.map(this._normalizeDimension.bind(this)),
            () => tensor._type?.shape?.dimensions?.map(this._normalizeDimension.bind(this)),

            // TensorFlow 格式
            () => tensor.shape?.dimensions?.map(this._normalizeDimension.bind(this)),
            () => tensor.tensorShape?.dim?.map((dim) => this._normalizeDimension(dim)),
            () => tensor.type?.shape?.dim?.map(this._normalizeDimension.bind(this)),
            ...
            () => {
                ...
                if (tensor.shape && typeof tensor.shape === 'object') {
                    return [this._normalizeDimension(tensor.shape)];
                }
                return null;
            },

            // TFLite 格式
            () => tensor.shapeSignature || tensor.shape,
            ...
            // 從名稱字串中解析（如："float32[1,256,256,3]"）
            () => {
                const tensorStr = tensor.toString?.() || '';
                const shapeMatch = tensorStr.match(/\[([0-9,?]+)\]/);
                if (shapeMatch) {
                    return shapeMatch[1].split(',').map((dim) => {
                        const trimmed = dim.trim();
                        return trimmed === '?' ? '?' : parseInt(trimmed, 10);
                    });
                }
                return null;
            }
    ...
    _parseDtype(tensor) {
        ...
        // 通用資料類型解析策略 - 基於實際 Netron 資料結構
        const dtypeExtractors = [
            // Netron 標準結構：tensor.arguments[0].type.dataType
            () => {
            ...
            // Netron Argument/Value 結構：argument.value -> [ Value ] 或 value 是單一 Value
            () => {
                if (Array.isArray(tensor.value) && tensor.value[0]?.type?.dataType) {
                    return tensor.value[0].type.dataType;
                }
                if (tensor.value && tensor.value?.type?.dataType) {
                    return tensor.value.type.dataType;
                }
                if (Array.isArray(tensor.value) && tensor.value[0]?.initializer?.type?.dataType) {
                    return tensor.value[0].initializer.type.dataType;
                }
                if (tensor.value && tensor.value?.initializer?.type?.dataType) {
                    return tensor.value.initializer.type.dataType;
                }
                return null;
            },

            // 從 Netron 張量字串解析（如："float32[1,256,256,3]"）
            () => {
                const tensorStr = tensor.toString?.() || '';
                const dtypeMatch = tensorStr.match(/^([a-zA-Z0-9_]+)(?:\[|$)/);
                if (dtypeMatch) {
                    return dtypeMatch[1];
                }
                return null;
    ...
    _suggestRuntimes(model) {
        ...
        switch (format) {
            case 'onnx':
                ...
                // 選取可用的第一個 graph/module，兼容不同 factory 命名
                {
                    const graph = model.graphs?.[0] || model.modules?.[0] || model._modules?.[0] || {};
                    if (this._checkQuantization(graph).isQuantized) {
                        runtimes.push('qnn');
                    }
                }
                break;
    ...
    _extractMetadata(model) {
        ...
        if (model._metadata) {
            metadata.producer = metadata.producer || model._metadata.producer;
            metadata.version = metadata.version || model._metadata.version;
            metadata.domain = metadata.domain || model._metadata.domain;
            metadata.docString = metadata.docString || model._metadata.description;
        }
        if (model.description) {
            metadata.docString = metadata.docString || model.description;
        }
        return metadata;
    }
    // 用途與應用場景自動化抽取（可用於LLM prompt）
    _extractUsage(model) {
        // 可根據模型名稱、metadata、taskGuess等自動生成
        if (model.usage) {
            return model.usage;
        }
        if (model._metadata && model._metadata.usage) {
            return model._metadata.usage;
        }
        // 預設
        return '此模型可用於AI推論、物件偵測、分類、分割等場景。';
    }
    ...
    _extractApplication(model) {
        ...
        // 預設
        return ['自動駕駛', '監控系統', '零售分析', '機器人視覺', '增強現實'];
    }
