...
export class ModelSummarizer {
    ...
    summarize(model) {
        ...
        // 更詳細的檢查
        if (model) {
            console.log('🔍 [ModelSummarizer] model 是否存在:', !!model);
            console.log('🔍 [ModelSummarizer] model._modules 是否存在:', !!model._modules);
        ...
        if (model && model._modules && model._modules.length > 0) {
            ...
            console.error('  - model 存在:', !!model);
            console.error('  - model._modules 存在:', !!(model && model._modules));
            console.error('  - model._modules 長度:', model?._modules?.length);
            console.error('  - model.graphs 存在:', !!(model && model.graphs));
        ...
        const formatDetectors = [
            ...
            () => {
                if (model._version || model.version) {
                    ...
                    if (version.includes('ONNX')) return 'onnx';
                    if (version.includes('TensorFlow')) return 'tensorflow';
                    if (version.includes('PyTorch')) return 'pytorch';
                }
                return null;
            }
        ];
    ...
    _extractApplication(model) {
        ...
        if (model._metadata && model._metadata.application) return model._metadata.application;
        // 預設
