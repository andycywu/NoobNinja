...
export class ModelSummarizer {
    ...
    _extractInputs(graph) {
        ...
        if (graph.inputs) {
            for (const input of graph.inputs) {
                const inputInfo = {
                    name: input.name || 'input',
                    shape: this._extractShape(input),
                    dtype: this._extractDataType(input),
                    domain: this._guessDomain(input),
                    provenance: 'model',
                    confidence: 1.0
                };
    ...
    _extractOutputs(graph) {
        ...
        if (graph.outputs) {
            for (const output of graph.outputs) {
                const outputInfo = {
                    name: output.name || 'output',
                    shape: this._extractShape(output),
                    dtype: this._extractDataType(output),
                    semantics: this._guessSemantics(output),
                    provenance: 'model',
                    confidence: 1.0
                };
     ...
     */
    _guessDomain(input) {
        const shape = this._extractShape(input);
    ...
    _guessSemantics(output) {
        const name = output.name?.toLowerCase() || '';
        const shape = this._extractShape(output);

        if (name.includes('logit') || name.includes('score') || name.includes('prob')) {
            return 'logits';
        }
        if (name.includes('box') || name.includes('bbox')) {
            return 'bounding_boxes';
        }
        if (name.includes('mask') || name.includes('segment')) {
            return 'segmentation_mask';
        }
        if (name.includes('keypoint') || name.includes('pose')) {
            return 'keypoints';
        }

        // 基於形狀推斷
        if (shape.length === 2 && shape[1] >= 80 && shape[1] <= 1000) {
