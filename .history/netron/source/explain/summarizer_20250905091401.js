...
export class ModelSummarizer {
    ...
    _parseDtype(tensor) {
        ...
        // 1. 標準結構: tensor.type.dataType
        if (tensor.type?.dataType) {
            console.log('🔧 [ModelSummarizer] 從 type.dataType 獲得類型:', tensor.type.dataType);
            return tensor.type.dataType;
        }

        // 2. 嘗試從 dataType 屬性
        if (tensor.dataType) {
            console.log('🔧 [ModelSummarizer] 從 dataType 屬性獲得類型:', tensor.dataType);
            return tensor.dataType;
        }

        // 3. 嘗試從 _type 私有屬性
        if (tensor._type?.dataType) {
            console.log('🔧 [ModelSummarizer] 從 _type.dataType 獲得類型:', tensor._type.dataType);
            return tensor._type.dataType;
        }
        ...
        if (tensor.arguments && tensor.arguments.length > 0) {
            ...
            if (arg.type?.dataType) {
                console.log('🔧 [ModelSummarizer] 從 arguments 獲得類型:', arg.type.dataType);
                return arg.type.dataType;
            }
        }

        // 5. 嘗試從 dtype 屬性（某些格式使用）
        if (tensor.dtype) {
            console.log('🔧 [ModelSummarizer] 從 dtype 屬性獲得類型:', tensor.dtype);
            return tensor.dtype;
        }
