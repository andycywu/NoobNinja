...
view.ExplainSidebar = class extends view.ObjectSidebar {
    ...
    constructor(context, model, explainEngine) {
        ...
        // 監聽重新掃描事件
        document.addEventListener('explain-rescan', (event) => {
            this._handleRescan();
        });
    }
    ...
    _renderLoading() {
        ...
        loadingDiv.innerHTML = `
            <div style="text-align: center; padding: 20px;">
                <div style="margin-bottom: 10px; font-size: 14px;">🔍 分析模型中...</div>
                <div style="font-size: 11px; color: #666; line-height: 1.4;">
                    正在解析模型結構、生成解釋與檢測 LLM 可用性
                </div>
    ...
    _renderError(error) {
        ...
        errorDiv.innerHTML = `
            <div style="text-align: center; padding: 20px;">
                ...
                <button class="explain-retry-btn" onclick="this.closest('.sidebar-object').querySelector('.explain-error').dispatchEvent(new CustomEvent('retry'))">
                    重試
                </button>
            </div>
        `;
    ...
    _renderSimpleContent() {
        ...
        // 輸入需求
        this.addSection('輸入需求');
        for (const input of summary.inputs) {
            this.addProperty(input.name, `${JSON.stringify(input.shape)} (${input.dtype})`);

            const req = explanation.inputRequirements?.find((r) => r.name === input.name);
            if (req && req.preprocessing && req.preprocessing.length > 0) {
                const preprocessDiv = this.createElement('div', 'sidebar-item-value');
                preprocessDiv.innerHTML = `
                    <div class="sidebar-item-value-line" style="font-size: 10px; color: #666;">
                        前處理: ${req.preprocessing.slice(0, 2).join('; ')}
                    </div>
                `;
                this.element.appendChild(preprocessDiv);
            }
        }

        // 輸出含義
        this.addSection('輸出說明');
        for (const output of summary.outputs) {
            this.addProperty(output.name, `${JSON.stringify(output.shape)} (${output.dtype})`);

            if (output.semantics && output.semantics !== 'unknown') {
                const semanticsDiv = this.createElement('div', 'sidebar-item-value');
                semanticsDiv.innerHTML = `
                    <div class="sidebar-item-value-line" style="font-size: 10px; color: #666;">
                        含義: ${this._translateSemantics(output.semantics)}
                    </div>
                `;
                this.element.appendChild(semanticsDiv);
            }
        }

        // 應用場景
        if (explanation.applications && explanation.applications.length > 0) {
            this.addSection('應用場景');
            const appsDiv = this.createElement('div', 'sidebar-item-value');
            const appsList = explanation.applications.slice(0, 5).map((app) => `• ${app}`).join('<br>');
            appsDiv.innerHTML = `<div class="sidebar-item-value-line" style="font-size: 10px;">${appsList}</div>`;
            this.element.appendChild(appsDiv);
        }

        // Dummy Run 結果
        if (dummyRunResult) {
            this.addSection('執行測試');
            if (dummyRunResult.success) {
                this.addProperty('推理時間', `${dummyRunResult.inferenceTime.toFixed(2)} ms`);
                this.addProperty('記憶體使用', `${(dummyRunResult.memoryUsage.total / 1024 / 1024).toFixed(2)} MB`);

                // 顯示實際輸出形狀
                for (const output of dummyRunResult.outputs) {
                    this.addProperty(`實際輸出 ${output.name}`, `[${output.shape.join(', ')}]`);
                }
            } else {
                this.addProperty('執行狀態', '失敗');
                if (dummyRunResult.error) {
                    const errorDiv = this.createElement('div', 'sidebar-item-value');
                    errorDiv.innerHTML = `<div class="sidebar-item-value-line" style="font-size: 10px; color: #d32f2f;">${dummyRunResult.error}</div>`;
                    this.element.appendChild(errorDiv);
                }
            }
        }
        ...
        if (summary.runtimeHints && summary.runtimeHints.length > 0) {
            this.addSection('建議執行環境');
            const runtimeDiv = this.createElement('div', 'sidebar-item-value');
            const runtimeList = summary.runtimeHints.slice(0, 4).join(', ');
            runtimeDiv.innerHTML = `<div class="sidebar-item-value-line" style="font-size: 10px;">${runtimeList}</div>`;
            this.element.appendChild(runtimeDiv);
        }

        // LLM 驗證結果
        if (validationResult && this._analysisResult.metadata.usedLLM) {
            this.addSection('AI 解釋品質');
            this.addProperty('信心度', `${(validationResult.confidence * 100).toFixed(0)}%`);
            if (validationResult.warnings.length > 0) {
                const warningDiv = this.createElement('div', 'sidebar-item-value');
                warningDiv.innerHTML = `<div class="sidebar-item-value-line" style="font-size: 10px; color: #ff9800;">⚠️ ${validationResult.warnings.length} 個注意事項</div>`;
                this.element.appendChild(warningDiv);
            }
        }
    ...
    async _exportFullPackage() {
        try {
            ...
            if (Array.isArray(packageData)) {
                // 簡化版：依序下載所有檔案
                for (const file of packageData) {
                    this._downloadFile(file.name, file.content, 'text/plain');
                    await new Promise((resolve) => setTimeout(resolve, 100)); // 避免同時下載
                }
            } else {
                // ZIP 格式
...
view.SettingsSidebar = class extends view.ObjectSidebar {
    ...
    async render() {
        try {
            ...
            // 載入設定 UI
            if (!this._settingsUI) {
                const { SettingsUI } = await import('./ui/settings.js');
                this._settingsUI = new SettingsUI(this._settingsManager);
            }
            ...
            await this._settingsUI.render(settingsContainer);

            // 監聽設定變更
            this._settingsUI.on('settingsChanged', () => {
                // 通知主 View 設定已變更
                if (this._view._explainSidebar) {
                    this._view._explainSidebar.refresh();
                }
            });
